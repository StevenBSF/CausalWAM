...
class Motus(nn.Module):
    ...
    def __init__(self, config: MotusConfig):
        ...
        # Optional experiment-owned action-token conditioner.  It stays None
        # for every native Motus run, so the released model/state-dict contract
        # is unchanged unless an experiment explicitly installs the adapter.
        self.policy_content_conditioner = None
    ...
    def set_policy_content_conditioner(self, conditioner: Optional[nn.Module]) -> None:
        """Install or remove an optional Policy Content Adapter.

        The conditioner API is deliberately minimal to keep Motus independent
        from experiment code: it must expose ``inject_action_tokens``.  Passing
        ``None`` restores the exact native action path.
        """
        if conditioner is not None and not callable(
            getattr(conditioner, "inject_action_tokens", None)
        ):
            raise TypeError(
                "policy content conditioner must expose inject_action_tokens"
            )
        self.policy_content_conditioner = conditioner

    def _inject_policy_content_tokens(
        self,
        action_tokens: torch.Tensor,
        policy_content_tokens: Optional[torch.Tensor],
    ) -> torch.Tensor:
        conditioner = self.policy_content_conditioner
        if conditioner is None:
            if policy_content_tokens is not None:
                raise RuntimeError(
                    "policy content tokens were supplied without an installed conditioner"
                )
            return action_tokens
        return conditioner.inject_action_tokens(
            action_tokens, policy_content_tokens
        )
    ...
    def training_step(
        ...
        policy_content_tokens: Optional[torch.Tensor] = None,
        ...
        action_tokens = self._inject_policy_content_tokens(
            action_tokens, policy_content_tokens
        )
        ...
        with torch.autocast(device_type="cuda", dtype=self.video_model.precision):
            ...
            if compute_video_loss:
                video_pred = self.video_module.apply_output_head(
                    video_tokens, video_head_time_emb
                )
                # Video loss (mask the first teacher-forced frame).
                video_pred_masked = video_pred.clone()
                video_pred_masked[:, :, 0:1] = 0
                video_loss = torch.nn.functional.mse_loss(
                    video_pred_masked, video_target, reduction='mean'
                )
            else:
                # Policy continuation is action-only.  A detached scalar keeps
                # the return schema stable without opening a video-loss path
                # into Action/Understanding experts through joint attention.
