...
class WanVideoModel(nn.Module):
    ...
    def get_layer_features(
        ...
        stop_after_last_requested: bool = False,
    ) -> List[torch.Tensor]:
        """
        ...
        if stop_after_last_requested and not layer_indices:
            raise ValueError("stop_after_last_requested requires a non-empty layer_indices list")
        ...
        with torch.autocast(
            ...
            for i, block in enumerate(self.wan_model.blocks):
                ...
                if stop_after_last_requested and i >= max(layer_indices):
                    break

        if stop_after_last_requested:
