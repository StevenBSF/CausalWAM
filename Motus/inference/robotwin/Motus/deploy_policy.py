...
from .policy_content_adapter import (
...
from .qwen_processor import load_qwen3_vl_processor
...
class MotusPolicy:
    ...
    def __init__(self, checkpoint_path: str, config_path: str, wan_path: str, vlm_path: str, device: str = "cuda", log_dir: Optional[str] = None, task_name: Optional[str] = None, adapter_checkpoint_path: Optional[str] = None):
        self.device = device
        ...
        self.vlm_processor = load_qwen3_vl_processor(self.vlm_path)
    ...
    def _load_model(self) -> Motus:
        ...
        try:
            ...
            model.load_checkpoint(self.checkpoint_path, strict=True)
        ...
        if self.adapter_checkpoint_path:
            adapter_path = Path(self.adapter_checkpoint_path).resolve()
            if not adapter_path.is_file():
                raise FileNotFoundError(
                    f"Policy Content Adapter checkpoint not found: {adapter_path}"
                )
            payload = torch.load(adapter_path, map_location="cpu", weights_only=False)
            if payload.get("schema") != "motus_policy_content_adapter_deployment_checkpoint":
                raise ValueError("Invalid Policy Content Adapter checkpoint schema")
            if payload.get("regime") not in {"m_p1", "m_p2"}:
                raise ValueError("Invalid Policy Content Adapter regime")
            conditioner = MotusPolicyContentConditioner(capture_layer=16).to(
                device=self.device, dtype=model.dtype
            )
            conditioner.load_state_dict(payload["conditioner"], strict=True)
            if payload["regime"] == "m_p2":
                if payload.get("action_expert") is None:
                    raise ValueError("M-P2 adapter checkpoint omitted Action Expert")
                model.action_expert.load_state_dict(
                    payload["action_expert"], strict=True
                )
            elif payload.get("action_expert") is not None:
                raise ValueError("M-P1 adapter checkpoint unexpectedly contains Action Expert")
            ...
            logger.info(
                "Loaded Policy Content Adapter: %s",
                self.adapter_metadata,
            )
        else:
    ...
    def get_action(self, instruction: str = None) -> List[np.ndarray]:
        ...
        with torch.no_grad():
            ...
            if self.model.policy_content_conditioner is not None:
                visual_tokens = extract_observation_visual_tokens(
                    self.model,
                    first_frame=current_frame,
                    language_embeddings=t5_list,
                    capture_layer=self.model.policy_content_conditioner.capture_layer,
                )
                policy_content_tokens = (
                    self.model.policy_content_conditioner.content_tokens(
                        visual_tokens
                    )
                )
            predicted_frames, predicted_actions = self.model.inference_step(
                ...
                policy_content_tokens=policy_content_tokens,
            )
...
def get_model(usr_args):
    ...
    adapter_checkpoint_path = usr_args.get('adapter_checkpoint_path') or os.environ.get(
        'MOTUS_ADAPTER_CHECKPOINT'
    )
