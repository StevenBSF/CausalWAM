...
class Base_Task(gym.Env):
    ...
    def _init_task_env_(self, table_xy_bias=[0, 0], table_height_bias=0, **kwags):
        ...
        self.content_seed = kwags.get("content_seed", kwags.get("seed", 0))
        self.style_seed = kwags.get("style_seed", None)
        self.background_texture_ids = kwags.get("background_texture_ids", None)
        self.render_device_alias = kwags.get("render_device_alias", None)
        self._pair_trace_hook = None
        self._pair_trace_semantic_action = None
    ...
    def setup_scene(self, **kwargs):
        ...
        if self.render_device_alias is None:
            # Preserve RoboTwin's native scene path for ordinary callers.  The
            # explicit path below is used only by the paired collector.
            ...
            self.render_device_info = None
        else:
            # SAPIEN's Engine compatibility path chooses a Vulkan device
            # implicitly.  Resolve the requested physical GPU by PCI address
            # and attach it to the scene explicitly for strict multi-GPU runs.
            sapien.render.set_camera_shader_dir("rt")
            sapien.render.set_ray_tracing_samples_per_pixel(32)
            sapien.render.set_ray_tracing_path_depth(8)
            sapien.render.set_ray_tracing_denoiser("oidn")
            ...
            render_device = sapien.Device(self.render_device_alias)
            if not render_device.can_render():
                raise RuntimeError(
                    f"SAPIEN device {self.render_device_alias!r} cannot render"
                )
            render_system = sapien.render.RenderSystem(render_device)
            self.scene = sapien.Scene(
                [sapien.physx.PhysxCpuSystem(), render_system]
            )
            actual_device = self.scene.render_system.device
            visible_devices = os.environ.get("CUDA_VISIBLE_DEVICES")
            declared_physical_gpu = os.environ.get("ROBOTWIN_PHYSICAL_GPU_INDEX")
            expected_pci = os.environ.get("ROBOTWIN_EXPECTED_GPU_PCI")
            if (
                declared_physical_gpu is None
                or not declared_physical_gpu.isdigit()
                or visible_devices != declared_physical_gpu
                or expected_pci is None
                or self.render_device_alias != f"pci:{expected_pci}"
            ):
                raise RuntimeError(
                    "explicit SAPIEN rendering requires matching CUDA GPU index "
                    "and nvidia-smi PCI provenance environment values"
                )
            pci_string = actual_device.pci_string
            if not isinstance(pci_string, str):
                raise RuntimeError(
                    f"SAPIEN device {self.render_device_alias!r} has no PCI address"
                )
            pci_string = pci_string.lower()
            if pci_string != expected_pci.lower():
                raise RuntimeError(
                    "SAPIEN RenderSystem selected the wrong physical GPU: "
                    f"expected PCI {expected_pci}, actual PCI {pci_string}"
                )
            self.render_device_info = {
                "requested_alias": self.render_device_alias,
                "physical_gpu_index": int(declared_physical_gpu),
                "cuda_visible_devices": visible_devices,
                "name": str(actual_device.name),
                "logical_cuda_id": int(actual_device.cuda_id),
                "pci_string": pci_string,
                "can_render": bool(actual_device.can_render()),
            }
            if (
                actual_device.cuda_id != render_device.cuda_id
                or actual_device.pci_string != render_device.pci_string
            ):
                raise RuntimeError(
                    "SAPIEN RenderSystem did not retain the requested device: "
                    f"requested={render_device}, actual={actual_device}"
                )
            ...
            # Paired collection uses render_freq=0.  Keep explicit-device
            # interactive use functional without reintroducing Engine.
            self.engine = None
            self.renderer = None
            if self.render_freq:
                self.renderer = sapien.render.SapienRenderer(actual_device)
    ...
    def create_table_and_wall(self, table_xy_bias=[0, 0], table_height=0.74):
        ...
        if self.random_background:
            ...
            if self.background_texture_ids is not None:
                if self.clean_background_rate != 0:
                    raise ValueError("background_texture_ids requires clean_background_rate == 0")
                if not isinstance(self.background_texture_ids, (list, tuple)) or len(
                        self.background_texture_ids) != 2:
                    raise ValueError("background_texture_ids must contain [wall_id, table_id]")

                texture_ids = []
                for texture_id in self.background_texture_ids:
                    if isinstance(texture_id, (bool, np.bool_)) or not isinstance(texture_id, (int, np.integer)):
                        raise TypeError("background texture IDs must be integers")
                    texture_id = int(texture_id)
                    texture_path = os.path.join(directory_path, f"{texture_id}.png")
                    if not os.path.isfile(texture_path):
                        raise FileNotFoundError(f"Background texture does not exist: {texture_path}")
                    texture_ids.append(texture_id)

                wall_texture, table_texture = texture_ids
                self.background_texture_ids = tuple(texture_ids)
    ...
    def set_pair_trace_hook(self, hook):
        """Install a hook used to compare demonstration physics and saved frames."""
        self._pair_trace_hook = hook

    def clear_pair_trace_hook(self):
        """Disable pair tracing without affecting normal environment execution."""
        self._pair_trace_hook = None

    def _notify_pair_trace_hook(self, event, **payload):
        hook = self._pair_trace_hook
        if hook is None:
            return
        callback = hook.get(event) if isinstance(hook, dict) else getattr(hook, event, None)
    ...
    def _pair_trace_scene_step(self, semantic_action):
        previous_action = self._pair_trace_semantic_action
        self._pair_trace_semantic_action = semantic_action
        try:
            self._notify_pair_trace_hook("before_step")
            self.scene.step()
            self._notify_pair_trace_hook("after_step")
        finally:
            self._pair_trace_semantic_action = previous_action
    ...
    def get_obs(self):
        ...
        if self.data_type.get("true_qpos", False) or self.data_type.get("true_qvel", False):
            pkl_dic["robot_state"] = {}
            if self.data_type.get("true_qpos", False):
                pkl_dic["robot_state"]["left_qpos"] = np.asarray(self.robot.left_entity.get_qpos()).copy()
                pkl_dic["robot_state"]["right_qpos"] = np.asarray(self.robot.right_entity.get_qpos()).copy()
            if self.data_type.get("true_qvel", False):
                pkl_dic["robot_state"]["left_qvel"] = np.asarray(self.robot.left_entity.get_qvel()).copy()
                pkl_dic["robot_state"]["right_qvel"] = np.asarray(self.robot.right_entity.get_qvel()).copy()
        # pointcloud
    ...
    def _take_picture(self):  # save data
        ...
        pkl_path = self.folder_path["cache"] + f"{frame_index}.pkl"
        save_pkl(pkl_path, pkl_dic)  # use cache
        self.FRAME_IDX += 1
        self._notify_pair_trace_hook("on_frame_saved", frame_index=frame_index, pkl_path=pkl_path)
    ...
    def together_move_to_pose(
        ...
        while now_left_id < left_n_step or now_right_id < right_n_step:
            ...
            self._pair_trace_scene_step("together_move_to_pose")
    ...
    def take_dense_action(self, control_seq, save_freq=-1):
        ...
        for control_idx in range(max_control_len):
            ...
            self._pair_trace_scene_step("take_dense_action")
    ...
    def take_action(self, action, action_type:Literal['qpos', 'ee']='qpos'):  # action_type: qpos or ee
        ...
        while now_left_id < left_n_step or now_right_id < right_n_step:
            ...
            self._pair_trace_scene_step("take_action")
            self._update_render()
