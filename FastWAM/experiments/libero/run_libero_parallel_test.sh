...
run_libero_eval() {
    ...
    launch_task_on_pane() {
        ...
        tmux send-keys -t $SESSION_NAME:$pane_info "source ~/.bashrc && conda activate fastwam-gl && export PYTHONPATH=\"$ROOT_DIR/third_party/libero:\$PYTHONPATH\"  && cd $ROOT_DIR && export EXP_NAME=$EXP_NAME && \
            STATUS_FILE='$status_file' LOG_FILE='$log_file' RESULT_FILE='$result_file' && \
            MUJOCO_GL=osmesa PYOPENGL_PLATFORM=osmesa CUDA_VISIBLE_DEVICES=$gpu_id /root/anaconda3/envs/fastwam-gl/bin/python experiments/libero/eval_libero_single.py \
